...
- 説明、比較、構造、時系列、状態遷移では、重要な関係が文章や短い一覧より materially 理解しやすくなり、`visualize` skillが利用可能な場合に使用する。
- 単純な事実、一段階の操作、短い列挙には可視化を追加しない。形式と実装手順は`visualize` skillをsource of truthとする。
