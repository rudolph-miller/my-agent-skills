...
`dev` フローを変えるときは、必要に応じて次を一緒に更新する。`~/projects/my-agent-skills` を master とし、master から配備先・ミラーへ同期する。
...
master（真実の情報源）は `~/projects/my-agent-skills`。配備先は次のとおり。
...
master から各 skill ディレクトリ単位で同期する。`skills/` root 全体に `--delete` をかけない。

重要: Codex は `~/.codex/skills` と `~/.agents/skills` の両方を走査し、同名スキルが両方に実体で存在すると、名前での暗黙呼び出しが「曖昧」として無効化される。実体は必ず `~/.agents/skills` の1箇所だけに置き、他はsymlinkにする。
...
1. **現状確認**
   - `~/projects/my-agent-skills/dev/SKILL.md` を読む
   - `~/projects/my-agent-skills/update-dev-skills/SKILL.md` を読む
...
```bash
python ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py ~/projects/my-agent-skills/dev
python ~/.codex/skills/.system/skill-creator/scripts/quick_validate.py ~/projects/my-agent-skills/update-dev-skills
```
